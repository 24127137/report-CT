...
A recommendation system is an information filtering technology that predicts users' preferences and suggests relevant items from large item spaces. These systems are fundamental to modern digital services, addressing information overload by personalizing content discovery across e-commerce, entertainment, social media, and professional networks.

\subsection{Core Components and Definitions}
\cite{survey}
\cite{Kumar2024}
\begin{itemize}
    \item \textbf{Users ($U$):} The set of all users in the system, where each user $u \in U$ has associated features or profile data.
    \item \textbf{Items ($I$):} The set of recommendable items, where each item $i \in I$ has associated attributes or content features.
    \item \textbf{Interactions ($R$):} The set of observed user-item interactions, typically represented as a sparse matrix $R \in \mathbb{R}^{|U| \times |I|}$.
    \item \textbf{Context ($C$):} Additional information about the recommendation scenario (time, location, device, etc.).
    \item \textbf{Recommendation Task:} Given historical interactions $R$ and optional context $C$, predict future interactions or rank items by relevance.
\end{itemize}

\subsection{Evaluation Metrics}
Common metrics for assessing recommendation quality include:
\begin{itemize}
    \item \textbf{Rating Prediction:} RMSE (Root Mean Square Error), MAE (Mean Absolute Error)
    \item \textbf{Ranking Quality:} Precision@k, Recall@k, NDCG (Normalized Discounted Cumulative Gain)
    \item \textbf{Diversity:} Category coverage, Intra-list diversity
    \item \textbf{Novelty:} Average popularity of recommended items
\end{itemize}
...
\end{itemize}
Output: Latent factor vectors for users and items. The final score is the dot product of these vectors.

Comparison: MF solves the sparsity problem by inferring hidden preferences and improves scalability by reducing dimensionality. However, it often struggles to model non-linear relationships or integrate side features easily.
...
\hline
Multilayer perceptron-based recommendation & Feed-forward neural networks with multiple hidden layers & User and item features, interaction data & High computational cost, requires large training data \\
\hline
...
\section{Specialized Recommendation Approaches}
